...
#    def gsearch(self):
#        self.params["svc"] = {'kernel':('linear', 'rbf'),
#                              'C':[1, 10], 
#                              'gamma':[.1, .01]}
#        self.params["mlp"] = {}
#        
#        self.params["lr"] = {'C':[10, 1.0, 0.1, 0.01, 0.001]}
#        self.params['gpc'] = {}
#        self.params['gnb'] = {}
#        self.params['sgd'] = {}
#        self.params['rndmfrst'] = {'n_estimators':[400],
#                                   ["min_sample_leaf":[1,5,10,50]]}
#        self.params['gboost'] = {}
#        

if __name__ == "__main__": 
    
    model = Image(sys.argv[1], sys.argv[2], sys.argv[3])
        
    def pickler(filename, model): 
        filepath = "/".join(filename.split("/")[:-1])
        print filepath + "/" + model.column_name + "_model.pkl"
        now = dt.strftime(dt.now(),"%Y-%m-%d %H:%M")
        with open(filepath + "/" + model.column_name +"_"+ now 
                  + "_model.pkl", "wb") as model_name:
            pickle.dump(model, model_name)                
    
    pickler(model.filename, model)
...
##
##def get_targets():
##    target_filepath = "/Users/danielbenton/Galvanize/capstone_ideas/finance"\
##                        "/project/data/target_data/"
##    if "target_df.pkl" in os.listdir(target_filepath): 
##        print "Found 'target_df.pkl.  Opening now"
##        target_df = open_pickle(target_filepath + "target_df.pkl")
##    else: 
##        target_df = globber(target_filepath)
##    return target_df
#
#def build_target():
#    target_filepath = "/Users/danielbenton/Galvanize/capstone_ideas/finance"\
#                        "/project/data/target_data/"
#    
#    df = open_pickle(target_filepath + "target_df.pkl")
#    return df
#
#def pickler(filename, obj): 
#    with open(filename, "wb") as model_name: 
#        pickle.dump(obj, model_name)
#        
...
#    df = df.dropna(axis=0)
#    df2_len = df.shape[0]
#    print "{} total rows dropped. Saturation != 100%".format(df_len - df2_len)
#    return df
#    
#def modeler(n): 
#    svd = Pipeline([("tfidfvec", TfidfVectorizer()),  
...
#    df = df[col]
#    df = na_dropper(df)
#    model = modeler(df.shape[0])
#    model.fit(df)   
#    transformed = model.transform(df)
#    return pd.DataFrame(transformed, index=df.index)
#
#def regularizer(df, col):
#    df1 = na_dropper(df[[col,"threshold"]])
#    y = df1.pop("threshold")
#    model = modeler(df1.shape[0]) 
#    srs = df1[col]
#    #fit svd model to max n == number of days in df
#    model.fit(srs)
#    transformed = model.transform(srs)
#    #variable selection.  Iterate through alphas and store coefs
#    #dataframe of latent features by col
#    trans_df = pd.DataFrame(columns = np.arange(len(transformed)))
#    
#    for i in np.linspace(1, .01, 51): 
#        L1 = LogisticRegression(penalty="l1", C=i)
#        L1.fit(transformed, y.values)
#        trans_df = trans_df.append(pd.DataFrame(L1.coef_, index=[i]))
#    coef_df = pd.DataFrame()
#    for i in range(len(trans_df)): 
#        coefs = trans_df.iloc[i][abs(trans_df.iloc[i]) > 0]
...
#            coef_df = coef_df.append(coefs.sort_values(ascending=False))
#    coef_df = coef_df.fillna(0)    
#    ax = coef_df.plot(title="Value of " + col + " Coefficients as Alpha "\
#                             "Approches Zero.",figsize=(10,10))
#    fig = ax.get_figure()   
#    fig.savefig("/Users/danielbenton/Galvanize/capstone_ideas/finance/"\
#                "project/data/visuals/" + col + "_Coeff_alpha.png")
#    X = pd.DataFrame(transformed)[list(coef_df.columns)].set_index(y.index)
#    return pd.concat([X, y], axis=1)   
#
#if __name__ == "__main__":     
#    filepath = "/Users/danielbenton/Galvanize/capstone_ideas/finance/project/"\
#                "data/training_data/"
#    filename = "source_df.pkl"
#    source_df = open_pickle(filepath + filename)
#    
#    col_list = list(source_df.columns)
#    y_df = build_target()
#    merged_df = source_df.merge(y_df, how="inner", left_index=True, 
#                            right_index=True)
#    drop_list = ["targetbuy", "std", "threshold_multiclass"]
#    merge_df = merged_df.drop(drop_list, axis=1)
#    data_filepath = "/Users/danielbenton/Galvanize/capstone_ideas/finance/"\
#                    "project/data/final_data/"
#    for col in col_list:
#        data_set = regularizer(merge_df, col)
#        pickler(data_filepath + col +"_data_set.pkl", data_set) 
#        print "Pickled " + col + "_data_set.pkl"
##        
#    